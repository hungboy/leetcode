...
var serialize = function(root) {
  if (root) {
    ...
    while (queue.length > 0) {
      const { node: currentNode, index: currentIndex } = queue.shift();

      if (currentNode) {
        if (currentNode.left) {
          queue.push({
            node: currentNode.left,
            index: calculateLeftIndex(currentIndex)
          });
        }
        if (currentNode.right) {
          queue.push({
            node: currentNode.right,
            index: calculateRightIndex(currentIndex)
          });
        }
        tree[currentIndex] = currentNode.val;
      }
    }
    ...
    for (let val of tree) {
      nulledTree.push(typeof val === 'undefined' ? 'null' : val);
    }
...
const isLeftChild = index => index % 2 === 1;

const calculateParentIndex = index => Math.floor((index - 1) / 2);

const calculateLeftIndex = index => 2 * index + 1;

const calculateRightIndex = index => 2 * index + 2;
...
var deserialize = function(data) {
  ...
  console.log('made it past data array');
  dataArray.forEach((value, index) => {
    dataArray[index] = value === 'null' ? null : new TreeNode(value);
    if (index) {
      const parentIndex = calculateParentIndex(index);
      const isLeft = isLeftChild(index);

      if (dataArray[parentIndex]) {
        dataArray[parentIndex][isLeft ? 'left' : 'right'] = dataArray[index];
      }
    }
  });

  console.log('made it past deserialization');
  return dataArray[0];
};
